...
export async function getMincePieMakersForYear(year: number): Promise<Result<Maker[], StorageError>> {
  try {
    ...
    const ids = pieResult.rows.reduce<Set<string>>((acc, row) => {acc.add(row.get('makerid')); return acc; }, new Set<string>());
    const result = await CASSANDRA_CLIENT.execute("SELECT * FROM mincepierank.maker WHERE id in ?;", [Array.from(ids)], { prepare: true });
